          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              {data.kospi && (
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <div className="text-2xl font-bold">{data.kospi.current_price.toLocaleString()}</div>
                  </div>
                  <div className="text-right">
                    <div className={`flex items-center gap-1 ${
                      data.kospi.change > 0 ? 'text-red-600' :
                      data.kospi.change < 0 ? 'text-blue-600' : 'text-gray-500'
                    }`}>
                      {data.kospi.change > 0 ? <TrendingUp className="w-4 h-4" /> :
                       data.kospi.change < 0 ? <TrendingDown className="w-4 h-4" /> : null}
                      <span className="font-medium">
                        {data.kospi.change > 0 ? '+' : ''}{data.kospi.change.toFixed(2)}
                      </span>
                    </div>
                    <div className={`text-sm ${
                      ...
                      ({data.kospi.change > 0 ? '+' : ''}{data.kospi.change_rate.toFixed(2)}%)
                    </div>
                  </div>
                </div>
              )}
              ...
              {data.kosdaq && (
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <div className="font-medium">KOSDAQ</div>
                    <div className="text-2xl font-bold">{data.kosdaq.current_price.toLocaleString()}</div>
                  </div>
                  <div className="text-right">
                    <div className={`flex items-center gap-1 ${
                      data.kosdaq.change > 0 ? 'text-red-600' :
                      data.kosdaq.change < 0 ? 'text-blue-600' : 'text-gray-500'
                    }`}>
                      {data.kosdaq.change > 0 ? <TrendingUp className="w-4 h-4" /> :
                       data.kosdaq.change < 0 ? <TrendingDown className="w-4 h-4" /> : null}
                      <span className="font-medium">
                        {data.kosdaq.change > 0 ? '+' : ''}{data.kosdaq.change.toFixed(2)}
                      </span>
                    </div>
                    <div className={`text-sm ${
                      data.kosdaq.change > 0 ? 'text-red-600' :
                      ...
                      ({data.kosdaq.change > 0 ? '+' : ''}{data.kosdaq.change_rate.toFixed(2)}%)
                    </div>
                  </div>
                </div>
              )}
